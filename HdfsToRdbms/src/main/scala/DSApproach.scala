...
object DSApproach {
  ...
  def main(args: Array[String]) {
    ...
    //Importing spark implicits._ for SparkSession scoped instance
    // to use inferring by reflection
    ...
    val top10perCategory =
      productsPerCategory
      .withColumn("row_num", row_number.over(partitionedByCategory))
      .where($"row_num" <= 10)
    ...
    top10perCategory
      .write
      .mode("overwrite")
      .jdbc(url, top10productsInCat, prop)
    ...
    val geoData =
      ...
      .toDF("ip_address", "geoname_id", "country_name", "product_price")
      .as[CountryExpense]

    //Getting top n country spendings
    val topCountrySpendings =
      geoData
    ...
    topCountrySpendings
      ...
      .write
      .mode("overwrite")
      .jdbc(url, top10spendingsspark, prop)
  }
  //Event class as a Encoder for deserialization
  case class Event(product_name: String,
  ...
  case class CountryExpense(ip_address: String,
                            geoname_id: Int,
                            country_name: String,
                            product_price: Int
                           )
